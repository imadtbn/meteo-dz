...
export default function Home({ forcedCity, detail = false }: HomeProps) {
  ...
  return <main dir="rtl">
    <header className="site-header"><Link href="/" className="brand"><img src="/manus-storage/atlas-mark_78817ed0.png" alt="رمز الطقس الجزائري" /><span>الطقس <b>الجزائري</b></span></Link><nav aria-label="التنقل الرئيسي"><a href="#now">الآن</a><a href="#outlook">التوقعات</a><a href="#regions">المدن</a><a href="#marine">البحر</a></nav><a className="header-source" href="https://open-meteo.com/" target="_blank" rel="noreferrer">مصدر البيانات <ArrowUpLeft size={15} /></a></header>

    <section className="hero-band">
      <img className="hero-art" src="/manus-storage/atlas-hero-weather_edadd9be.jpg" alt="خريطة مناخية تجريدية للجزائر والبحر المتوسط" />
      ...
      <div className="city-command" aria-label="اختيار المدينة"><MapPinned size={20} /><select value={city.slug} onChange={(event) => selectCity(event.target.value)} aria-label="اختر مدينة"><option value="" disabled>اختر مدينة</option>{ALGERIAN_CITIES.map((item) => <option key={item.slug} value={item.slug}>{item.name} — {item.region}</option>)}</select><button onClick={locate} className="icon-button dark" aria-label="تحديد أقرب مدينة"><LocateFixed size={20} /></button></div>
    </section>
    ...
    <section className="regions-section" id="regions"><div className="dashboard-shell"><div className="section-intro"><span>04 — المدن</span><h2>انتقل مباشرة إلى مدينتك</h2><p>9 محطات عرض مختارة · روابط ثابتة للمشاركة والفهرسة.</p></div><div className="region-grid">{ALGERIAN_CITIES.map((item) => <Link key={item.slug} href={`/weather/${item.slug}`} className="region-card"><span>{item.region}</span><b>{item.name}</b><ArrowLeft size={18} /></Link>)}</div><AdSlot slot="8546947691" /></div></section>
